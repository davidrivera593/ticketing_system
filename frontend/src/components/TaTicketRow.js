...
export default function TaTicketRow({ ticket, onOpen, escalated, showCheckboxes, selectedCheckboxes, onToggleSelect }) {
    const {
        ticket_id,
        issue_type,
        issue_description,
    ...
    return (
        <Box
            ...
            sx={{
                display: "grid",
                gridTemplateColumns: showCheckboxes ? "40px 30px 0px 1fr 2fr 1fr 2fr 100px 100px" : "30px 0px 1fr 2fr 1fr 2fr 100px 100px",
                gap: 2,
                px: 2,
                py: 1.5,
                alignItems: "center",
            ...
            aria-label={`Open ticket for ${ta_name}`}
        >
            {/*Checkbox Column*/}
            {showCheckboxes && (
                <span>
            ...
            {/* Avatar Column */}
            <Avatar sx={{ width: 32, height: 32 }}>{initials(ta_name)}</Avatar>
            ...
            <Box sx={{ minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <Typography variant="body2" fontWeight={600} noWrap title={ta_name}>
                    {ta_name }
                </Typography>
                {escalated && (
                    <Chip
            ...
            {/* Team Column */}
            <Typography variant="body2" noWrap title={student_name} sx={{ color: "text.primary", textAlign: "center" }}>
                {student_name}
            </Typography>
